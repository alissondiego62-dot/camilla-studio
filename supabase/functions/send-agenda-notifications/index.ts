...
Deno.serve(async(request)=>{
  ...
  return new Response(JSON.stringify(error?{error:error.message}:{created:data,message:"Alertas centralizados gerados; execute dispatch-notifications para entrega push."}),{status:error?500:200,headers});
});
