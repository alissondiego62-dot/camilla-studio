...
export function ProjectActivitiesPanel({ projectId, activities, users, canCreate, canEdit, onChanged }: { projectId: string; activities: ProjectActivity[]; users: ProjectOption[]; canCreate: boolean; canEdit: boolean; onChanged: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = new FormData(event.currentTarget); const userId = String(form.get("responsible_user_id") || "") || null; const user = users.find((item) => item.id === userId);
    const result = await action.run(() => addProjectActivity(projectId, { title: String(form.get("title") || "").trim(), description: String(form.get("description") || "").trim() || null, due_date: String(form.get("due_date") || "") || null, responsible_user_id: userId, responsible_name: user?.name ?? null }), "Atividade criada.");
    if (result.ok) { setOpen(false); await onChanged(); }
  }
