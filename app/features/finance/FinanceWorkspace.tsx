...
import { FinanceEnvironmentSwitcher } from "./FinanceEnvironmentSwitcher";
...
const environments=new Set<FinanceEnvironment>(["personal","professional","consolidated"]);
...
export function FinanceWorkspace(){
 ...
 const initialEnvironment=environments.has(params.get("environment") as FinanceEnvironment)?params.get("environment") as FinanceEnvironment:"professional";
 ...
 const canProfessional=workspace.permissions.can("finance_professional","view");const canPersonal=workspace.permissions.can("finance_personal","view");const canConsolidated=canProfessional&&canPersonal&&workspace.permissions.can("finance_professional","view_consolidated");
 ...
 function changeEnvironment(next:FinanceEnvironment){workspace.setEnvironment(next);workspace.setPage(0);updateUrl(next,workspace.section,null)}
 ...
 return <ModuleFrame title="Financeiro" subtitle="Ambientes pessoal e profissional separados por RLS, com visão consolidada autorizada">
  <FinanceEnvironmentSwitcher value={workspace.environment} onChange={changeEnvironment} personal={canPersonal} professional={canProfessional} consolidated={canConsolidated}/>
  <FinanceNavigation active={workspace.section} onChange={changeSection}/>
  <FinanceToolbar filters={workspace.filters} onFilters={(value)=>{workspace.setFilters(value);workspace.setPage(0)}} onPeriod={workspace.changePeriod} options={workspace.data.options} environment={workspace.environment} onNew={()=>openEntry("new")} onTransfer={()=>workspace.setTransferOpen(true)} onRecurrence={()=>void workspace.generateRecurrence()} onReload={()=>void workspace.reload()} canCreate={workspace.canCreate&&workspace.environment!=="consolidated"} canTransfer={canTransfer} canRecurrence={canRecurrence}/>
  <FeedbackMessage error={workspace.action.error} success={workspace.action.success}/>{workspace.error&&<ErrorState message={workspace.error} onRetry={()=>void workspace.reload()}/>} {workspace.loading?<LoadingState/>:<>
   ...
   {workspace.section==="revenue"&&<FinanceRevenue items={workspace.visibleEntries} canViewValues={workspace.canViewValues} canSettle={workspace.canSettle} onOpen={openEntry} onSettle={workspace.setPaymentEntry}/>} 
   {workspace.section==="expenses"&&<FinanceExpenses items={workspace.visibleEntries} canViewValues={workspace.canViewValues} canSettle={workspace.canSettle} onOpen={openEntry} onSettle={workspace.setPaymentEntry}/>} 
   {workspace.section==="receivables"&&<FinanceReceivables items={workspace.visibleEntries} canViewValues={workspace.canViewValues} canSettle={workspace.canSettle} onOpen={openEntry} onSettle={workspace.setPaymentEntry}/>} 
   {workspace.section==="payables"&&<FinancePayables items={workspace.visibleEntries} canViewValues={workspace.canViewValues} canSettle={workspace.canSettle} onOpen={openEntry} onSettle={workspace.setPaymentEntry}/>} 
   ...
   {workspace.section==="accounts"&&workspace.environment!=="consolidated"&&<FinanceCatalogManager title="Contas" description="Contas bancárias, caixa físico, carteiras e contas digitais." table="financial_accounts" items={workspace.data.options.accounts} environment={workspace.storedEnvironment} canManage={canManageAccounts} onReload={()=>void workspace.reload()} typeOptions={[["bank","Conta bancária"],["cash","Caixa físico"],["wallet","Carteira"],["digital","Conta digital"],["investment","Investimento"],["other","Outro"]]}/>} 
   {workspace.section==="cards"&&workspace.environment!=="consolidated"&&<FinanceCatalogManager title="Cartões" description="Cartões pessoais ou empresariais sem armazenar dados sensíveis completos." table="financial_cards" items={workspace.data.options.cards} accounts={workspace.data.options.accounts} environment={workspace.storedEnvironment} canManage={canManageCards} onReload={()=>void workspace.reload()}/>} 
   {workspace.section==="templates"&&workspace.environment!=="consolidated"&&<FinanceTemplatesWorkspace environment={workspace.storedEnvironment} options={workspace.data.options} canManageTemplates={canManageTemplates} canManageRecurrence={canRecurrence} onReload={()=>void workspace.reload()}/>} 
   {workspace.section==="auxiliary"&&workspace.environment!=="consolidated"&&<FinanceAuxiliaryRecords environment={workspace.storedEnvironment} options={workspace.data.options} canManage={canManageAux} onReload={()=>void workspace.reload()}/>} 
