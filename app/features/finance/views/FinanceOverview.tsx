...
export function FinanceOverview({data,canViewValues,canApprove,onReview}:{data:FinanceWorkspaceData;canViewValues:boolean;canApprove:boolean;onReview:(id:string,decision:"approved"|"rejected",reason:string)=>void}){
 const metrics=[['Saldo disponível',data.metrics.current_balance],['Recebido no período',data.metrics.realized_income],['A receber total',data.metrics.receivable],['A receber com data',data.metrics.receivable_dated],['A receber sem data',data.metrics.receivable_undated],['Valores vencidos',data.metrics.overdue],['Despesas pagas no período',data.metrics.realized_expense],['Contas a pagar',data.metrics.payable],['Resultado do período',data.metrics.net_result],['Projeção de caixa',data.metrics.projected_balance]];
