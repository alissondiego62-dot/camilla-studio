...
import { archiveFinanceEntry, cancelFinanceEntry, changeFinanceEnvironment, createFinanceTransfer, createInstallmentEntries, duplicateFinanceEntry, generateRecurringEntries, reactivateFinanceEntry, reviewFinanceApproval, saveFinanceEntry, settleFinanceEntry } from "./finance.mutations";
...
const empty: FinanceWorkspaceData = {entries:[],total:0,metrics:{current_balance:"0.00",expected_income:"0.00",realized_income:"0.00",expected_expense:"0.00",realized_expense:"0.00",net_result:"0.00",receivable:"0.00",payable:"0.00",overdue:"0.00",projected_balance:"0.00",previous_period_result:"0.00",result_change_percent:null},timeline:[],categories_chart:[],statuses_chart:[],options:{categories:[],subcategories:[],accounts:[],cards:[],suppliers:[],cost_centers:[],payment_methods:[],clients:[],projects:[],templates:[],recurring_rules:[]},approvals:[],access:[]};
...
export function useFinanceWorkspace(initialEnvironment:FinanceEnvironment="professional",initialSection:FinanceSection="overview"){
 ...
 async function cancel(id:string,reason:string){const result=await action.run(()=>cancelFinanceEntry(id,reason),"Lançamento cancelado.");if(result.ok){setEntryId(null);await load()}}
 ...
 return{permissions,action,environment,setEnvironment,section,setSection,filters,setFilters,changePeriod,page,setPage,pageSize,setPageSize,pageCount,data,visibleEntries,loading,error,reload:load,entryId,setEntryId,selectedEntry,paymentEntry,setPaymentEntry,installmentEntry,setInstallmentEntry,transferOpen,setTransferOpen,storedEnvironment,moduleName,canViewValues,canCreate,canEdit,canSettle,canExport,save,duplicate,archive,reactivate,cancel,settle,installment,moveEnvironment,reviewApproval,transfer,generateRecurrence};
}
