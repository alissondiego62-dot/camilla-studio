...
export async function settleProjectReceivable(projectId:string,input:Omit<FinancePaymentInput,"entry_id">&{description:string}){const result=await supabase.rpc("settle_project_receivable",{p_project_id:projectId,p_payload:{...input,amount:normalizeMoneyInput(input.amount),discount_amount:normalizeMoneyInput(input.discount_amount),interest_amount:normalizeMoneyInput(input.interest_amount),fine_amount:normalizeMoneyInput(input.fine_amount)}});assertNoError(result);return result.data as FinanceEntryRow}
