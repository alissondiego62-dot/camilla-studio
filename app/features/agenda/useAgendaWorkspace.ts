...
const emptyOptions:AgendaOptions={projects:[],users:[],statuses:[],snapMinutes:15};
export function useAgendaWorkspace(){
 const permissions=usePermissions();const[state,dispatch]=useReducer(agendaReducer,emptyAgendaState);const[options,setOptions]=useState(emptyOptions);const[mode,setMode]=useState<AgendaMode>("month");const[anchor,setAnchor]=useState(todayKey());const[filters,setFilters]=useState<AgendaFilters>(defaultAgendaFilters);const[loading,setLoading]=useState(true);const[error,setError]=useState("");
 ...
 const items=useMemo(()=>allItems.filter(item=>(filters.showCancelled||item.status!=="cancelled")&&(!filters.responsibleId||item.responsible_user_id===filters.responsibleId)&&(!filters.projectId||item.project_id===filters.projectId)&&(!filters.itemType||item.item_type===filters.itemType)&&(!filters.status||item.status===filters.status)),[allItems,filters]);
