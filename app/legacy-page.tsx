...
import { ArchitectureAgendaCalendar } from "./components/ArchitectureAgendaCalendar";
import { DriveIntegrationSettings } from "./components/DriveIntegrationSettings";
...
export default function Home() {
    ...
    <div className="studio-shell">
      ...
      <main className="studio-content">
        ...
        {activeView === "projects" && <>
          ...
          <section className="project-board" ref={boardRef} onScroll={(event) => synchronize(event.currentTarget, topScrollRef.current)}>{kanbanStages.map((stage) => <div className="project-column" key={stage} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const id = event.dataTransfer.getData("text/plain"); const project = projects.find((item) => item.id === id); if (project) void moveProject(project, stage); }}><header><div><i /><b>{stageLabels[stage]}</b></div><span>{filteredProjects.filter((project) => project.stage === stage).length}</span></header><div className="project-lane">{filteredProjects.filter((project) => project.stage === stage).map((project) => <article className="project-card" key={project.id} draggable onDragStart={(event) => event.dataTransfer.setData("text/plain", project.id)} onClick={() => setSelectedProject(project)}><div className="project-card-cover">{project.cover_url ? <img src={project.cover_url} alt={`Miniatura de ${project.name}`} /> : <span>⌂</span>}</div><div className="card-top"><b>{project.code}</b><span data-priority={project.priority}>{priorityLabels[project.priority]}</span></div><h3>{project.name}</h3><p>{projectClient(project)}</p><div className="card-meta"><span>{project.project_type}</span><span data-overdue={deadlineLabel(project).includes("atrasado")}>{deadlineLabel(project)}</span></div><footer><i>{(project.responsible_name || "CA").slice(0, 2).toUpperCase()}</i><span>{statusLabels[project.status]}</span></footer><div className="project-card-actions"><button type="button" onClick={(event) => { event.stopPropagation(); setSelectedProject(project); setDetailTab("agenda"); }}>Agenda</button><button type="button" onClick={(event) => { event.stopPropagation(); setSelectedProject(project); setDetailTab("files"); }}>Arquivos</button><button type="button" onClick={(event) => { event.stopPropagation(); setSelectedProject(project); setDetailTab("history"); }}>Histórico</button></div></article>)}</div></div>)}</section>
        </>}
  ...
  .map((item) => <article key={`${item.project.id}-${item.key}`}><time><b>{new Date(`${item.date}T12:00:00`).getDate()}</b><span>{new Date(`${item.date}T12:00:00`).toLocaleDateString("pt-BR", { month: "short" })}</span></time><div><small>{item.label}</small><h3>{item.project.name}</h3><p>{projectClient(item.project)}</p></div><button onClick={() => setSelectedProject(item.project)}>Abrir</button></article>)}</div></article></section>
        ...
        {activeView === "activities" && <ActivitiesWorkspace projects={projects} />}
        ...
        {activeView === "settings" && <div className="settings-page-grid">
          <section className="settings-command-center">
            <article><header><div><p className="eyebrow">PLATAFORMA</p><h3>Diagnóstico do sistema</h3></div><span>◆</span></header><ul><li>Supabase: {isSupabaseConfigured ? "conectado" : "não configurado"}</li><li>Projetos carregados: {projects.length}</li><li>Eventos de agenda: {calendarEvents.length}</li><li>Perfil atual: {profile?.camilla_role || "demonstração"}</li></ul><button>Executar diagnóstico</button></article>
            <article><header><div><p className="eyebrow">ACESSOS</p><h3>Usuários e permissões</h3></div><span>♙</span></header><ul><li>Administrador com acesso integral</li><li>Gerente de projetos com acesso operacional</li><li>Colaborador limitado aos projetos atribuídos</li><li>Financeiro e relatórios protegidos</li></ul><button>Gerenciar permissões</button></article>
            <article><header><div><p className="eyebrow">OPERAÇÃO</p><h3>Preferências do escritório</h3></div><span>⚙</span></header><ul><li>Etapas e status do Kanban</li><li>Modelos de checklist por tipo de projeto</li><li>Responsáveis e grupos de atividades</li><li>Categorias de arquivos e agenda</li></ul><button>Configurar operação</button></article>
          </section>
          <section className="settings-grid"><article><span>◆</span><div><small>BANCO DE DADOS</small><b>{isSupabaseConfigured ? "Supabase conectado" : "Aguardando configuração"}</b><p>Dados estruturados no Supabase com permissões por projeto.</p></div></article><article><span>▣</span><div><small>PROJETOS</small><b>{projects.length} registros</b><p>Kanban, agenda, atividades, checklists e arquivos integrados.</p></div></article><article><span>↗</span><div><small>MINIATURAS</small><b>Espaço preparado</b><p>Capas nos cards e prévias para PDFs, imagens e renders.</p></div></article></section><DriveIntegrationSettings /><NotificationSettings user={user} /></div>}
      </main>
